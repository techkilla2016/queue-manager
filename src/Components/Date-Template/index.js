...
import InsertTemplate from '@/Components/model/insertDateTemplate'
...
const DateTemplate = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            formatter: (cell, row) => {
                return (
                    <div className='modelImage'>
                        <Image src={row?.template} width={1920} height={1080} />
                    </div>
                )
            }
        },
        {
            ...
            formatter: (cell, row) => {
                return (
                    <>
                        <Button variant="primary mx-3" onClick={() => handleButtonClick(row.id)}>
                            update
                        </Button>
                        <Button variant="danger" onClick={() => handleButtonClick(row.id)}>
                            delete
                        </Button>
                    </>
                )
    ...
    useEffect(() => {
        axios.get(`https://mobile-server-ebon.vercel.app/date-template`)
            .then(res => {
